...
// When the accept queue is full the onEstablished goroutine must not block indefinitely
// (goroutine leak). Instead the connection is closed and the client observes an error
// on its next read.
func TestTCPListenerAcceptQueueFull(t *testing.T) {
	loggerFactory := logging.NewDefaultLoggerFactory()
	...
	// Pre-fill the accept queue to capacity so the next completed handshake
	// cannot enqueue its connection.
	tln, ok := ln.(*TCPListener)
	require.True(t, ok, "expected TCPListener")
	queueCap := cap(tln.acceptCh)
	for range queueCap {
		tln.acceptCh <- &TCPConn{}
	}

	// Dial one more connection. After the 3-way handshake completes on the
	// server, onEstablished sees the full queue and must close the connection
	// immediately (non-blocking select default branch) rather than blocking.
	conn, err := clientNet.DialTCP(tcp4, nil, &net.TCPAddr{IP: srvAddr.IP, Port: srvAddr.Port})
	require.NoError(t, err)
	defer func() { _ = conn.Close() }()

	// The server-side connection was closed (FIN sent), so Read must return an
	// error and not block forever.
	require.NoError(t, conn.SetReadDeadline(time.Now().Add(2*time.Second)))
	buf := make([]byte, 1)
	_, err = conn.Read(buf)
	require.Error(t, err, "Read must return an error — connection should have been rejected by server")
}
