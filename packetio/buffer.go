...
func (b *Buffer) writeByte(x byte) {
	b.data[b.tail] = uint8(x) //nolint:gosec
	b.tail++
...
// the caller should make sure the input buffer (in []byte) can be completely accomodated in b.data
...
func (b *Buffer) WriteWithAttributes(packet []byte, attr transport.PacketAttributesBuffer) (int, error) { //nolint:cyclop
	pLen := len(packet)
	aLen := 0
	if attr != nil {
...
func (b *Buffer) ReadWithAttributes(packet []byte, a transport.PacketAttributesBuffer) (n int, err error) { //nolint:gocognit,cyclop
	...
	for {
		...
		if b.head != b.tail { //nolint:nestif
			...
			if aLen > 0 {
				if a != nil {
					b.writeToInputBuffer(a.GetBuffer(), aLen)
				}
				b.advanceHead(aLen)
			}
			...
			if copied < count {
				// todo amir: it seems like this method still consumes the buffer even in the cases where
				// packet buffer is short. but even in this case copies into the packet buffer and discards it :|
